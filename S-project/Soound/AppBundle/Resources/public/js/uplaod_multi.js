...
var supported_types = Array('jpg','png','gif','jpeg');
...
$('.form1').ajaxForm({    
    ...
    uploadProgress: function(event, position, total, percentComplete) {
        var percentVal = percentComplete ;
            bar.width(percentVal)
             ...
             percent.html("uploading <img src='http://localhost/soound/web/bundles/sooundapp/css/images/trash.png'/>");  
    ...
    complete: function(xhr) {
        ...
        $('.upload_status').hide();
        percent.html("done");
        percent.html("done <img src='http://localhost/soound/web/bundles/sooundapp/css/images/trash.png'/>");
        percent.css('left','77%');
        ...
        //var file = $(this).siblings('span:first').text();
            ...
            if(confirm('Are you sure you wanna delete '+file+'?')){
                var ajax = $.ajax({ url: 'http://localhost/soound/web/bundles/sooundapp/upload/_delete.php',
                data: {file: file},
                type: 'POST',
                async: false
                });
                    if (ajax.responseText == "success") {
                        ...
                        $('.upload_bar').hide();
                        $('.upload_percent').hide();
                        $('.project_uploaded_img_wrapper').css('background','url("http://localhost/soound/web/bundles/sooundapp/css/images/plus.png") no-repeat center');
                    }
                }
                else {
                }
